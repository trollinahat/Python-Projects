...
class Checkers:
    ...
    def CompTurn(s):
        ...
        #Promotes moves which trade your pawn for opponent king -- regardless of risk
        for move in s.moves:
            if ((s.tiles[move[0]][move[1]].isKing) or (s.tiles[move[2]][move[3]].isKing) and not s.isMoveSafe(move)):
                s.badMoves.append(move)
        s.removeBadMoves()
    ...
    #Checks both the current move and next move for danger
    def isMoveSafe(s,move):
        X1,Y1 = [s.moveEndsAt(move)[0]-1,s.moveEndsAt(move)[0]+1],[s.moveEndsAt(move)[1]-1,s.moveEndsAt(move)[1]+1]
        X2,Y2 = [s.moveEndsAt(move)[0]-2,s.moveEndsAt(move)[0]+2],[s.moveEndsAt(move)[1]-2,s.moveEndsAt(move)[1]+2]
        for i in range(2):
            for j in range(2):
                if s.SpecialPCCP(s.tiles[move[0]][move[1]].pieceColour,X1[i],Y1[j],s.moveEndsAt(move)[0],s.moveEndsAt(move)[1],move[0],move[1]):
                    return False
                    for end2x in range(2):
                        for end2y in range(2):
                            if s.SpecialPCCP(s.tiles[move[0]][move[1]].pieceColour,X1[end2x],Y1[end2y],s.moveEndsAt(move)[0],s.moveEndsAt(move)[1],move[0],move[1]):
                                return False
        return True
